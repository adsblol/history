                    ...
                    try:
                        async with session.get(f"http://{ip}/aircraft.json") as resp:
                            ...
                            if len(app["aircrafts"]) > 30:
                                app["aircrafts"].pop(0)
                                print(
                                    "aircrafts.json list is too long, removing the oldest one"
                                )
        ...
        while True:
            ...
            for aircraft in app["aircrafts"][::-1]:
                ...
                for tier, (max_age, min_age) in tiers.items():
                    if (
                        now - app["last_stored_at"] > min_age
                        and now - app["last_stored_at"] < max_age
                    ):
                        candidate = aircraft
                        print(
                            f'tier={tier} now={now} last_stored_at={app["last_stored_at"]}'
                        )
                        break
            if not candidate:
            ...
            candidate_datetime_str = candidate_datetime.strftime(
                "%Y/%m/%d/%H/adsblol-%Y-%m-%d-%H-%M-%S.json"
            )
